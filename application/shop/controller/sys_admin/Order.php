    ...
    public function shipping()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($kd_type == 3) {
                ...
                $kdn_appid = $this->shipping_model->getVal(['name' => 'kdn_appid'], 'data');
                $kdn_apikey = $this->shipping_model->getVal(['name' => 'kdn_apikey'], 'data');
                $kdn_apiurl = $this->shipping_model->getVal(['name' => 'kdn_apiurl'], 'data');
                $kdn_name = $this->shipping_model->getVal(['name' => 'kdn_name'], 'data');
                $kdn_phone = $this->shipping_model->getVal(['name' => 'kdn_phone'], 'data');
                $kdn_province = $this->shipping_model->getVal(['name' => 'kdn_province'], 'data');
                $kdn_city = $this->shipping_model->getVal(['name' => 'kdn_city'], 'data');
                $kdn_area = $this->shipping_model->getVal(['name' => 'kdn_area'], 'data');
                $kdn_address = $this->shipping_model->getVal(['name' => 'kdn_address'], 'data');
