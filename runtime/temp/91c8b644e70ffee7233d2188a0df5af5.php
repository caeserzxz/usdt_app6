<?php /*a:4:{s:80:"D:\phpStudy\WWW\mainshop\application\weixin\view\sys_admin\reply_text\index.html";i:1552460091;s:69:"D:\phpStudy\WWW\mainshop\application\mainadmin\view\layouts\base.html";i:1552460091;s:79:"D:\phpStudy\WWW\mainshop\application\weixin\view\sys_admin\reply_text\list.html";i:1552460091;s:69:"D:\phpStudy\WWW\mainshop\application\mainadmin\view\layouts\page.html";i:1552460091;}*/ ?>
            ...
            <form id="forms" class="talbe-search form-inline "  method="post" action="<?php echo url('getList'); ?>" >
             <div class="page-breadcrumbs">
                    <ul class="breadcrumb" >
                      ...
                      <a href="<?php echo url('info'); ?>" data-toggle="ajaxModal" title="添加素材" class="btn btn-sm btn-default fr m-t-md m-r"><i class="fa fa-plus m-r-xs"></i>添加文本素材</a>
                </div>
            </form>
        </header>
...
<tr>
    ...
    <th width="200" align="center">修改时间</th>
...
<?php if(is_array($data['list']) || $data['list'] instanceof \think\Collection || $data['list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
    <tr>
     <td class="with-checkbox"><?php echo htmlentities($vo['id']); ?></td>
      ...
      <td align="center"><a href="#" data-remote="<?php echo url('ajaxEdit',array('id'=>$vo['id'],'field'=>'status')); ?>" class="<?php echo htmlentities(tplckval($vo['status'],'=1','active')); ?>"  data-toggle="class" data-ajax="true"><i class="fa fa-check text-success text-active"></i><i class="fa fa-times text-danger text"></i></a></td>
      <td align="center"><a href="#" title="设为关注回复，用户关注后优先收到的信息" data-remote="<?php echo url('ajaxEdit',array('id'=>$vo['id'],'field'=>'subscribe')); ?>" class="<?php echo htmlentities(tplckval($vo['subscribe'],'=1','active')); ?>"  data-toggle="class" data-ajax="true"><i class="fa fa-check text-success text-active"></i><i class="fa fa-times text-danger text"></i></a></td>
      <td align="center"><a href="#" title="设为默认回复，用户输入没有的关键字时的自动回复" data-remote="<?php echo url('ajaxEdit',array('id'=>$vo['id'],'field'=>'default')); ?>" class="<?php echo htmlentities(tplckval($vo['default'],'=1','active')); ?>" data-toggle="class" data-ajax="true"><i class="fa fa-check text-success text-active"></i><i class="fa fa-times text-danger text"></i></a></td>
    <td>
    <a href="<?php echo url('info',array('id'=>$vo['id'])); ?>"  data-toggle="ajaxModal" title="编辑" class="fa fa-edit m-xs" ></a>
    <a href="<?php echo url('delete',array('id'=>$vo['id'])); ?>" data-toggle="ajaxRemove" data-msg="确定删除 <?php echo htmlentities($vo['keyword']); ?>"  class="fa fa-trash"  title="删除"></a>
    </td>
    </tr>
