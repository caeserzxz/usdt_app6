<?php /*a:4:{s:75:"D:\phpStudy\WWW\moduleshop\application\shop\view\sys_admin\bonus\index.html";i:1549953095;s:71:"D:\phpStudy\WWW\moduleshop\application\mainadmin\view\layouts\base.html";i:1550818706;s:74:"D:\phpStudy\WWW\moduleshop\application\shop\view\sys_admin\bonus\list.html";i:1549953095;s:71:"D:\phpStudy\WWW\moduleshop\application\mainadmin\view\layouts\page.html";i:1549953095;}*/ ?>
    ...
    <form id="forms" class="talbe-search form-inline "  method="post" action="<?php echo url('getList'); ?>" >
         <div class="page-breadcrumbs">
                <ul class="breadcrumb" >
                 ...
                 <select name="send_type" style="width: 150px;" data-toggle="select2" data-chang="submit">
                        <option value="-1">发放类型</option>
                     	<?php if(is_array($SendType) || $SendType instanceof \think\Collection || $SendType instanceof \think\Paginator): $i = 0; $__LIST__ = $SendType;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$st): $mod = ($i % 2 );++$i;?>
                        	<option value="<?php echo htmlentities($st['ext_val']); ?>"><?php echo htmlentities($st['name']); ?></option>
                        <?php endforeach; endif; else: echo "" ;endif; ?>
                 </select>
                ...
                <a href="<?php echo url('info'); ?>" class="btn btn-sm btn-default fr m-t-md m-r"><i class="fa fa-plus m-r-xs"></i>新增优惠券</a>
...
<?php if(is_array($data['list']) || $data['list'] instanceof \think\Collection || $data['list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?>
        <tr>
            ...
            <td><span class="money"><?php echo htmlentities($row['type_money']); ?></span></td>
            <td><?php echo htmlentities($SendType[$row['send_type']]['name']); ?></td>
            <td><span class="money"><?php echo htmlentities($row['min_amount']); ?></span></td>
            <td><?php echo htmlentities($row['used_num']); ?></td>
            <td><?php echo htmlentities(dateTpl($row['add_time'])); ?></td>
            <td><?php echo htmlentities(dateTpl($row['use_end_date'])); ?></td>
            <td>
            	<?php if($row['send_type'] == '3'): ?><a href="<?php echo url('getExcel',array('type_id'=>$row['type_id']),false); ?>" class="m-xs" title="报表"><i class="fa fa-download text-muted"></i></a><a href="<?php echo url('send',array('type_id'=>$row['type_id'])); ?>" class="m-xs" title="发放"><i class="fa fa-random text-muted"></i></a><?php endif; if($row['send_type'] == '1'): ?><a href="<?php echo url('send',array('type_id'=>$row['type_id'])); ?>" class="m-xs" title="发放"><i class="fa fa-random text-muted"></i></a><?php endif; ?>
                <a href="<?php echo url('info',array('type_id'=>$row['type_id'])); ?>" class="m-xs" title="编辑"><i class="fa fa-edit text-muted"></i></a>
                <a href="<?php echo url('bonusList',array('type_id'=>$row['type_id'])); ?>" class="m-xs" title="查看"><i class="fa fa-search text-muted"></i></a>
