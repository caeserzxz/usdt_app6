<?php /*a:4:{s:83:"D:\phpStudy\WWW\moduleshop\application\shop\view\sys_admin\goods_comment\index.html";i:1551092090;s:71:"D:\phpStudy\WWW\moduleshop\application\mainadmin\view\layouts\base.html";i:1550818706;s:82:"D:\phpStudy\WWW\moduleshop\application\shop\view\sys_admin\goods_comment\list.html";i:1551165946;s:71:"D:\phpStudy\WWW\moduleshop\application\mainadmin\view\layouts\page.html";i:1549953095;}*/ ?>
        ...
        <div class="page-breadcrumbs">
            ...
            <label>
                <select name="status" style="width: 150px;" data-toggle="select2" data-chang="submit">
                    ...
                    <?php echo $statusListHtml; ?>
                </select>
            </label>
             <input type="text" class="form-control input-small"  id="goods_keyword" placeholder="商品名称/SN" value="" style="width:120px;" /> <a href="javascript:;" class="m-xs" title="查看"  onclick="searchgoods('goods_keyword','select_goods');"><i class="fa fa-search text-muted"></i></a>
              <select id="select_goods" name="select_goods"  style="width: 130px; " >
                  <option value="">选择商品</option>
              </select>
            <label>
              
                <input type="text" class="form-control input-large" name="keyword" value="<?php echo htmlentities($search['keyword']); ?>" placeholder="评论关键字/会员ID"/>
                <button class="btn btn-default-iq" type="submit"><i class="fa fa-search"></i></button>
            </label>
            <a href="<?php echo url('info'); ?>" class="btn btn-default fr m-t-md m-r"><i class="fa fa-plus m-r-xs"></i>添加商品评论</a>
        </div>
    </form>
</header>
...
<section class="scrollable wrapper w-f">
    ...
    <tbody>
        <?php if(is_array($data['list']) || $data['list'] instanceof \think\Collection || $data['list'] instanceof \think\Paginator): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?>
        <tr>
            <td style="text-align: center"><?php echo htmlentities($item['id']); ?></td>
            <td ><?php echo htmlentities($item['by_name']); ?></td>
            <td ><?php echo htmlentities($item['user_name']); ?></td>
            <td ><a href="javascript:;" class="<?php echo htmlentities(tplckval($item['is_imgs'],'=1','active')); ?>" ><i class="fa fa-check text-success text-active"></i><i class="fa fa-times text-danger text"></i></a></td>
            <td ><?php echo htmlentities(dateTpl($item['create_time'])); ?></td>
            <td ><a href="#" data-remote="<?php echo url('ajaxEdit',array('id'=>$item['id'],'field'=>'status')); ?>" class="<?php echo htmlentities(tplckval($item['status'],'=2','active')); ?>"  data-toggle="class" data-ajax="true"><i class="fa fa-check text-success text-active"></i><i class="fa fa-times text-danger text"></i></a></td>
             <td ><?php echo htmlentities(adminInfo($item['admin_id'])); ?></td>
            <td ><a href="<?php echo url('info',array('id'=>$item['id'])); ?>"  title="编辑" class="fa fa-edit m-xs" ></a></td>
        </tr>
         <tr>
            <td colspan="8">评论内容：<?php echo htmlentities($item['content']); ?></td>
        </tr>
        <?php endforeach; endif; else: echo "" ;endif; ?>
    </tbody>
</table>
...
<table width="100%">
    <tr>
        <td height="300" colspan="8" style="text-align: center" valign="middle">没有相关数据！</td>
    </tr>
</table>
<?php endif; ?>
        </div>
